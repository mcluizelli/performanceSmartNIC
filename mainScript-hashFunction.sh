...
#reading
    ...
    echo "bit<32> auxIndex1;" >> $P4FILENAME 

    if [ $1 -eq 0 ]
    then
         for i in $(seq $2) 
         do
             echo "hash(auxIndex1, HashAlgorithm.crc32, 32w0, {hdr.ipv4.srcAddr}, 32w99);" >> $P4FILENAME 
         done
    ...
    then
         for i in $(seq $2) 
         do
             echo "hash(auxIndex1, HashAlgorithm.crc16, 32w0, {hdr.ipv4.srcAddr}, 32w99);" >> $P4FILENAME 
         done
    ...
    then
         for i in $(seq $2) 
         do
             echo "hash(auxIndex1, HashAlgorithm.identity, 32w0, {hdr.ipv4.srcAddr}, 32w99);" >> $P4FILENAME 
         done
    ...
    then
         for i in $(seq $2) 
         do
             echo "hash(auxIndex1, HashAlgorithm.csum16, 32w0, {hdr.ipv4.srcAddr}, 32w99);" >> $P4FILENAME 
         done
    ...
    then
         for i in $(seq $2) 
         do
             echo "hash(auxIndex1, HashAlgorithm.crc32_custom, 32w0, {hdr.ipv4.srcAddr}, 32w99);" >> $P4FILENAME 
         done
    ...
    then
         for i in $(seq $2) 
         do
             echo "hash(auxIndex1, HashAlgorithm.crc16_custom, 32w0, {hdr.ipv4.srcAddr}, 32w99);" >> $P4FILENAME 
         done
    ...
    then
         for i in $(seq $2) 
         do
             echo "hash(auxIndex1, HashAlgorithm.random, 32w0, {hdr.ipv4.srcAddr}, 32w99);" >> $P4FILENAME 
         done
    ...
    then
         for i in $(seq $2) 
         do
             echo "hash(auxIndex1, HashAlgorithm.xor16, 32w0, {hdr.ipv4.srcAddr}, 32w99);" >> $P4FILENAME 
         done
...
getPktLatency(){
    ...
    nPackets="$(sudo /opt/netronome/p4/bin/rtecli -p 20206 registers get -r latency -i 0)"
  #  echo "${nPackets}"
...
configRemoteMachine(){
    ...
    #$hashType $hashNum $pktSize

    fileNameAux=$aux-pktSize-$3-numHash-$2
    ...
    echo $PATH_EXP/$aux-pktSize-$3-numHash-$2.txt

    mv $PATH_EXP/outAux.txt $PATH_EXP/$aux-pktSize-$3-numHash-$2.txt
...
main(){
    ...
    for hashType in 0 1 2 3 4 5 6 7
    do
       #number of hash function
       for hashNum in 0 1 5 10 15 20 25 30
       do 
           ...
           p4File_preambule $hashType $hashNum
           p4RegisterDeclaration $hashType $hashNum
           p4File_mid $hashType $hashNum
           ...
           /opt/netronome/p4/bin/rtecli -p 20206 design-load -f $PATH_EXP/code/firmware.nffw -c $PATH_EXP/myconfig.p4cfg 

           for pktSize in 64 128 256 512 1024 1500
           do
              echo "Running MG pktSize:" $pktSize
              configRemoteMachine $hashType $hashNum $pktSize
           done
