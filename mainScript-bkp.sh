   ...
   control MyIngress(inout headers hdr, inout metadata meta,
               ...
               // register<bit<64>>((bit<32>)2) latency;
                ...
                action ipv4_forward(macAddr_t dstAddr, egressSpec_t port) {
                     ...
                     hdr.ipv4.ttl = hdr.ipv4.ttl - 1;" >> $P4FILENAME
    
    if [ $i -ne 0 ]
    then
            echo "myRegister.write(0, (bit<$sizeRegister>)1);" >> $P4FILENAME
    fi
    aux=$1
    aux=$aux-1
    for i in $(seq $((aux)) )
    do
        echo "myRegister.write($i, (bit<$sizeRegister>)$i);" >> $P4FILENAME
    done
...
configRemoteMachine(){
    ...
    ssh -t root@200.132.136.69 '/opt/MoonGen/build/MoonGen /opt/MoonGen/examples/netronome-packetgen/packetgen.lua -tx 2 -rx 2 --dst-ip 10.1.0.10 --dst-ip-vary 0.0.0.0 --timeout 30 -fp outAux -ps '$size'' & #>/dev/null 
   #/opt/MoonGen/build/MoonGen /opt/MoonGen/examples/netronome-packetgen/packetgen.lua -tx 0 -rx 0 --dst-ip 10.1.0.10 --dst-ip-vary 0.0.0.0 -fp teste --timeout 1    
    ...
    scp root@200.132.136.69:/root/outAux.txt $PATH_EXP

    mv $PATH_EXP/outAux.txt $PATH_EXP/$aux-pktSize-$2-$sizeRegister.txt

    ssh -t root@200.132.136.69 'rm /root/outAux.txt'
    sleep 1
}
...
main(){
    ...
    for experiment in "read" "write" "read-write"
    do
        echo $experiment
    done 
